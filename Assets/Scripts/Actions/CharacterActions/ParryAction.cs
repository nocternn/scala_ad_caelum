using System.Collections;
using System.Collections.Generic;
using System.Threading.Tasks;
using UnityEngine;

[CreateAssetMenu(menuName = "Actions/Character Action/Parry")]
public class ParryAction : CharacterAction
{
    protected override void Awake()
    {
        type = Enums.CharacterActionType.Parry;
    }
    
    public override void PerformAction(EnemyManager enemy, bool playAnimation = true)
    {
        if (enemy.isInteracting)
            return;

        enemy.animatorHandler.PlayTargetAnimation(_animation, true);
        
        if (!enemy.isHit)
            return;
        
        Task.Delay((int)Mathf.Round(1000 * _duration)).ContinueWith(t =>
        {
            WeaponItem weapon = enemy.weaponSlotManager.GetCurrentWeapon();
            if (weapon.combatType == Enums.WeaponCombatType.Ranged)
            {
                weapon.PerformAttack(enemy.characterType, Enums.WeaponActionType.Shoot);
            }
            else
            {
                weapon.PerformAttack(enemy.characterType, Enums.WeaponActionType.Basic);
            }
        }, TaskScheduler.FromCurrentSynchronizationContext());
        
        base.PerformAction(enemy, playAnimation);
    }
}
